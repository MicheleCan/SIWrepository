...
class StudenteDaoJDBC implements StudenteDao {
	...
	public Studente findByPrimaryKey(String matricola) {
		...
		try {
			...
			if (result.next()) {
				...
				Indirizzo indirizzo = new IndirizzoDaoJDBC(this.dataSource).findByPrimaryKey(result.getLong("indirizzo_codice"));
	...
	public List<Studente> findAll() {
		...
		try {
			...
			while (result.next()) {
				...
				studente.setIndirizzo(indirizzoDao.findByPrimaryKey(result.getLong("indirizzo_codice")));
	...
	public void update(Studente studente) {
		...
		try {
			...
			statement.setLong(4, studente.getIndirizzo().getCodice());
