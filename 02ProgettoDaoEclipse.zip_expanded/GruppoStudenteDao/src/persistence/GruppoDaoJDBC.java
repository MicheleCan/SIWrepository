...
class GruppoDaoJDBC implements GruppoDao {
	...
	public Gruppo findByPrimaryKeyJoin(Long id) {
		...
		try {
			...
			String query = "select g.id as g_id, g.nome as g_nome, s.matricola as matricola,"
					+ "s.nome as nome, s.cognome as cognome, s.data_nascita as data_nascita,"
					+ "s.indirizzo_codice as indirizzo_codice from gruppo g left outer join "
					+ "studente s on g.id=s.gruppo_id where g.id = ?";
			...
			while (result.next()) {
				...
				if(result.getString("matricola")!=null){
					...
					// altra soluzione : doppio join Gruppo x Studente x Indirizzo
					IndirizzoDao indirizzoDao = new IndirizzoDaoJDBC(this.dataSource);
	...
	public List<Gruppo> findAllJoin() {
		Connection connection = this.dataSource.getConnection();
		HashMap<Long,Gruppo> gruppi = new HashMap();
		try {
			Gruppo gruppo;
			PreparedStatement statement;
			String query ="select g.id as g_id, g.nome as g_nome,"
					...
					+ "s.indirizzo_codice as indirizzo_codice from gruppo g "
					+ "left outer join studente s on g.id=s.gruppo_id ";
			...
			while (result.next()) {
				if(!gruppi.containsKey(result.getLong("g_id")) ) { 
				gruppo = new Gruppo();
				gruppo.setId(result.getLong("g_id"));				
				gruppo.setNome(result.getString("g_nome"));
				gruppi.put(gruppo.getId(),gruppo);				
				}
				if(result.getString("matricola")!=null) {
					...
					long date = result.getDate("data_nascita").getTime();
					s.setDataNascita(new Date(date));
					IndirizzoDao indirizzoDao = new IndirizzoDaoJDBC(dataSource);
					Indirizzo indirizzo = indirizzoDao.findByPrimaryKey(result.getLong("indirizzo_codice"));		
					s.setIndirizzo(indirizzo);
					gruppi.get(result.getLong("g_id")).addStudente(s);
				}
		...
		ArrayList<Gruppo> groups = new ArrayList<>(gruppi.values());
		return groups;
	}
